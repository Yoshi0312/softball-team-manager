...
## 10. 今後のバージョンアップ計画（2026年提案）

現状（Firebase認証 + チーム切替 + 既存機能モジュール化）を前提に、次の順で段階的にバージョンを上げる。

### v0.9.0（安定化リリース）
- 目的: 既存機能の品質確保（回帰防止）
- 実装項目:
  - 主要フローのE2Eテスト整備（ログイン、選手登録、成績保存、エクスポート）
  - `window` 公開APIの自動検証スクリプト化（inline `onclick` 互換の継続担保）
  - Firestore書き込み失敗時のUIエラーメッセージ統一
- 完了条件:
  - 主要フローの手動テストチェックリスト合格率100%
  - 既知の致命的バグ（データ消失・保存不可）0件

### v1.0.0（正式版）
- 目的: チーム運用で必要な最低機能を正式提供
- 実装項目:
  - 招待コード運用の改善（再発行、期限切れ表示、使用履歴）
  - ロール管理強化（admin/memberで編集可能範囲を明確化）
  - LocalStorage移行導線のUI改善（移行前プレビュー + バックアップ推奨）
  - Firebase Hosting本番運用手順の確立（リリース手順書化）
- 完了条件:
  - 3チーム以上で2週間の実運用テスト完了
  - 認証・権限系の重大インシデント0件
...
- 目的: 日常利用の継続率を上げる
- 実装項目:
  - 出欠管理機能（試合・練習イベントごとの参加可否）
  - 通知導線（未回答者の可視化、期限表示）
  - 保存済み試合・メンバー表の検索/フィルタ改善
- 完了条件:
  - 出欠機能を使ったイベント運用が月4回以上回る
  - 既存機能（選手/成績）の操作時間を悪化させない

### v1.2.0（分析・収益化準備）
- 目的: 有料化に向けた価値提供を開始
- 実装項目:
  - 成績分析の強化（期間比較、打順別傾向、PDFレポート試作）
  - チーム設定拡張（チームカラー、ヘッダー表示名、運用メモ）
  - 利用実績を計測するイベントログ設計（匿名化前提）
- 完了条件:
  - 指導者向けレポートを定例で出力できる
  - βユーザーへの有料プラン案内に必要な機能が揃う

### 開発運用ルール（全バージョン共通）
- 小さく出す: 1リリース1テーマを原則にする
- 先に壊れない仕組みを作る: 新機能より回帰テストを優先
- 互換性を維持する: 既存の画面操作（特にinline `onclick`）は段階的に移行
- データ保護を優先する: 破壊的変更時は必ずエクスポート導線を先に提供

---

## 12. 分析ダッシュボード v1 スコープ定義（2026年更新）
...
- スコープを以下4機能に限定する。
  ...
  - 月別試合数
  - 選手比較（主要5指標）
- 既存フィルタ（年度・試合種別）と必ず連動させる。

### v1 受け入れ条件
- 上記4機能が統計ページで表示され、年度・試合種別フィルタ変更時に同じ条件で再集計されること。
- 既存の成績一覧・選手成績表が回帰していないこと（手動チェックリストで確認）。

### v1.1（別チケット）
- v1完了後、以下を別チケットとして起票・着手する。
  - レーダーチャートの追加
  - 対戦相手別分析の追加
- チケット名（案）: `分析ダッシュボード v1.1: レーダーチャート / 対戦相手別分析`
